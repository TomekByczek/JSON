...
function energy() {
    ...
    for (var i = 0; i < pvData.pv.length - 1; i++) {
        var timeDiff = (new Date(pvData.pv[i + 1].creationTime) - new Date(pvData.pv[i].creationTime))
        if (fiveMinutes > timeDiff > 0) {
            kWhEnergy = (timeDiff / oneHour) * (pvData.pv[i].voltage * pvData.pv[i].current / 1000);
            sumEnergy = sumEnergy + kWhEnergy;
        }
        else {
            kWhEnergy = 60000 / oneHour * pvData.pv[i].voltage * pvData.pv[i].current / 1000;
            sumEnergy = sumEnergy + kWhEnergy;
        }
